...
export default function RealRailsDashboard() {
  ...
  if (!data) return (
    <div className="h-screen bg-[#030712] flex items-center justify-center text-[#38BDF8] font-mono animate-pulse">
      CALCULATING_DETERMINISTIC_RUNWAY...
    </div>
  );
  ...
  // --- NEW: DYNAMIC GHOST DETECTOR (Time-Aware) ---
  const dynamicInsights = data.sidebar.insights.map((sub: any) => {
    // Check how many times this specific ghost appeared in the selected time range
    const windowTxs = dynamicTransactions.filter((tx: any) => tx.name === sub.name);
    
    if (windowTxs.length === 0) return null; // If it didn't happen in this window, hide it

    return {
      ...sub,
      occurrencesInWindow: windowTxs.length,
      total_spent_window: windowTxs.reduce((sum: number, tx: any) => sum + tx.amount, 0)
    };
  }).filter(Boolean); // Remove nulls (ghosts that didn't appear)

  // Chart Data Preparation (Zero-Padding)
  ...
  expenseChartData.forEach((tx: any) => {
    if (dailyTotals[tx.date] !== undefined) dailyTotals[tx.date] += tx.amount;
  });
  ...
  while (currentSimBalance > 0 && dayCount <= 365) {
      ...
      simDate.setDate(simDate.getDate() + dayCount);
      const simDayOfMonth = simDate.getDate();
      currentSimBalance -= dailyNoiseRate;
      let billsToday = 0;
      let billsHit: any[] = [];
      if (recurringBillsByDay[simDayOfMonth]) {
          recurringBillsByDay[simDayOfMonth].forEach(bill => {
              billsToday += bill.amount;
              billsHit.push(bill);
          });
      ...
      forecastData.push({
          ...
          balance: Math.max(0, currentSimBalance),
          isBillDay: billsToday > 0,
          billsHit: billsHit,
          totalDrainedToday: dailyNoiseRate + billsToday
      });
      dayCount++;
  }
  ...
  return (
    <main className="flex h-screen bg-[#030712] overflow-hidden text-white font-sans selection:bg-[#38BDF8] selection:text-[#030712]">
      ...
      {selectedDate && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-[#030712]/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className={`bg-[#0B1117] border ${activeView === 'history' ? 'border-[#38BDF8]/50 shadow-[0_0_40px_rgba(56,189,248,0.15)]' : 'border-[#818CF8]/50 shadow-[0_0_40px_rgba(129,140,248,0.15)]'} rounded-2xl p-8 max-w-md w-full relative`}>
            ...
            <p className={`${activeView === 'history' ? 'text-[#38BDF8]' : 'text-[#818CF8]'} text-[10px] font-bold tracking-[0.2em] uppercase mb-1`}>
              {activeView === 'history' ? 'Daily Drilldown' : 'Forecasted Drain'}
            </p>
            ...
            <div className="space-y-3 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
              {activeView === 'history' && historyDrilldown.length > 0 ? (
                historyDrilldown.map((tx: any) => (
              ...
              ) : activeView === 'forecast' && forecastDrilldown ? (
                <>
                  <div className="flex justify-between items-center p-4 bg-[#030712] border border-[#1F2937] rounded-lg">
                    <div><p className="font-bold text-gray-300">Daily Average Spend</p></div>
                    <p className="font-mono font-black text-gray-400 text-lg">₹{dailyNoiseRate.toFixed(2)}</p>
                  </div>
                  {forecastDrilldown.isBillDay && forecastDrilldown.billsHit.map((bill: any, idx: number) => (
                    <div key={idx} className="flex justify-between items-center p-4 bg-[#030712] border border-[#ef4444]/50 rounded-lg shadow-inner">
                      <div><p className="font-bold text-white">{bill.name}</p></div>
                      <p className="font-mono font-black text-[#ef4444] text-lg">₹{bill.amount.toFixed(2)}</p>
                    </div>
                  ))}
                </>
              ) : <p className="text-gray-500 italic text-sm text-center py-4">No activity recorded.</p>}
            </div>
          </div>
        </div>
      )}
      ...
      <section className="w-[70%] p-10 border-r border-[#1F2937] flex flex-col relative overflow-y-auto">
        <header className="mb-10 shrink-0 flex justify-between items-end">
          ...
          <div className="flex bg-[#0B1117] border border-[#1F2937] p-1 rounded-lg">
            <button onClick={() => setActiveView('history')} className={`px-6 py-2 text-xs font-bold uppercase tracking-widest rounded transition-all ${activeView === 'history' ? 'bg-[#38BDF8] text-[#030712] shadow-[0_0_10px_rgba(56,189,248,0.5)]' : 'text-gray-500 hover:text-white'}`}>Historical Flow</button>
            <button onClick={() => setActiveView('forecast')} className={`px-6 py-2 text-xs font-bold uppercase tracking-widest rounded transition-all ${activeView === 'forecast' ? 'bg-[#818CF8] text-[#030712] shadow-[0_0_10px_rgba(129,140,248,0.5)]' : 'text-gray-500 hover:text-white'}`}>Runway Forecast</button>
          </div>
        </header>
        ...
        <div className="h-[400px] shrink-0 bg-[#0B1117] border border-[#1F2937] rounded-xl p-8 relative shadow-2xl overflow-hidden group mb-8">
          {activeView === 'history' ? (
            ...
            <div className="absolute top-6 left-8 right-8 z-10 flex justify-between items-start pointer-events-none">
              <div>
                <p className="text-[#818CF8] text-[10px] font-bold tracking-widest uppercase opacity-60">DETERMINISTIC_MODELING</p>
            ...
            <ResponsiveContainer width="100%" height="100%" className="cursor-pointer">
              {activeView === 'history' ? (
                ...
                <AreaChart data={forecastData} margin={{ top: 90, right: 20, left: -10, bottom: 0 }} onClick={(e: any) => { if (e && e.activeLabel) { setSelectedDate(e.activeLabel); } }}>
                  ...
                  <Tooltip contentStyle={{ backgroundColor: '#0B1117', border: '1px solid #1F2937', borderRadius: '8px' }} itemStyle={{ color: '#818CF8', fontWeight: 'bold' }} cursor={{ stroke: '#818CF8', strokeWidth: 1, strokeDasharray: "5 5" }} formatter={(value: number, name: string, props: any) => [`₹${value.toLocaleString()}`, props.payload.isBillDay ? 'Balance (Recurring Bill Hit!)' : 'Projected Balance']} />
        ...
        <div className="bg-[#0B1117] border border-[#1F2937] rounded-xl p-8 shadow-2xl shrink-0">
          <div className="flex justify-between items-end mb-6">
            <div><p className="text-[#38BDF8] text-[10px] font-bold tracking-widest uppercase opacity-60">Phase 1 Baseline</p><h3 className="text-2xl font-bold mt-1">Transaction Ledger</h3></div>
            <input type="text" placeholder="Search merchants..." className="bg-[#030712] border border-[#1F2937] text-white px-4 py-2 rounded-lg focus:outline-none focus:border-[#38BDF8] transition-colors w-72 text-sm placeholder-gray-600" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
          <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="text-[#818CF8] text-[10px] uppercase tracking-widest border-b border-[#1F2937]"><tr><th className="pb-3 font-bold">Date</th><th className="pb-3 font-bold">Merchant</th><th className="pb-3 font-bold">Category</th><th className="pb-3 text-right font-bold">Amount</th></tr></thead><tbody className="divide-y divide-[#1F2937]/50">
                {filteredTransactions.map((tx: any) => (
                  <tr key={tx.transaction_id} className="hover:bg-[#1F2937]/20 transition-colors group"><td className="py-4 text-gray-500 font-mono text-xs">{tx.date}</td><td className="py-4 font-bold text-gray-300 group-hover:text-white transition-colors">{tx.name}</td><td className="py-4"><span className={`px-2 py-1 bg-[#030712] border ${tx.category.includes('Income') ? 'border-green-400/50 text-green-400' : 'border-[#1F2937] text-gray-400'} rounded text-[9px] font-bold uppercase tracking-wider`}>{tx.category[0]}</span></td><td className={`py-4 text-right font-mono font-bold ${tx.category.includes('Income') ? 'text-green-400' : 'text-[#38BDF8]'}`}>{tx.category.includes('Income') ? '+' : ''}₹{tx.amount.toFixed(2)}</td></tr>
                ))}
          </tbody></table></div>
        </div>
      </section>
      ...
      <aside className="w-[30%] bg-[#0B1117] p-10 border-l border-[#1F2937] overflow-y-auto flex flex-col gap-10">
        <section><p className="text-[#38BDF8] text-[10px] font-bold tracking-[0.2em] mb-4 uppercase">Net Cashflow Gap</p>
          <div className="flex flex-col gap-4">
            <div><p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest mb-1">{timeRange}D Income</p><h2 className="text-4xl font-black text-green-400">₹{dynamicIncome.toLocaleString('en-IN')}</h2></div>
            <div><p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest mb-1">{timeRange}D Outflow</p><h2 className="text-4xl font-black text-white">₹{dynamicOutflow.toLocaleString('en-IN')}</h2></div>
          </div>
        </section>
        ...
        <section className="bg-[#030712] p-6 border border-[#1F2937] rounded-lg relative overflow-hidden group hover:border-[#818CF8]/50 transition-colors">
          ...
          <div className="flex items-baseline gap-2"><p className="text-white text-5xl font-black tracking-tighter">{dynamicRunway}</p><span className="text-gray-400 text-xs font-bold uppercase tracking-widest">Days Left</span></div>
        </section>

        <section className="flex-1">
          <h3 className="text-[#38BDF8] text-[10px] font-bold mb-5 uppercase tracking-[0.15em]">Ghost Rails Detected ({timeRange}D)</h3>
          <div className="space-y-4">
            {/* UPDATED: Mapping over dynamicInsights instead of static backend insights */}
            {dynamicInsights.length > 0 ? dynamicInsights.map((sub: any) => (
              <div key={sub.name} className="p-5 bg-[#030712] border border-[#1F2937] rounded-lg hover:border-[#38BDF8]/50 transition-all group cursor-pointer shadow-inner">
                <div className="flex justify-between items-center mb-1">
                  <p className="text-gray-500 text-[10px] font-bold uppercase tracking-tight">Recurring Leak ({sub.occurrencesInWindow}x)</p>
                ...
                <div className="flex justify-between items-end mt-2 border-t border-[#1F2937]/50 pt-2">
                  <div><p className="text-[9px] text-gray-500 uppercase tracking-widest mb-0.5">Per Charge</p><p className="text-xl font-black text-[#818CF8]">₹{sub.amount}</p></div>
                  <div className="text-right"><p className="text-[9px] text-red-500/80 font-bold uppercase tracking-widest mb-0.5">{timeRange}D Drain</p><p className="text-xl font-black text-red-400">₹{sub.total_spent_window.toFixed(2)}</p></div>
                </div>
              </div>
            )) : (
              <div className="p-5 bg-[#030712] border border-[#1F2937] border-dashed rounded-lg text-center"><p className="text-gray-600 text-xs italic uppercase tracking-widest">No recurring leaks detected in this window.</p></div>
            )}
          </div>
        </section>
      </aside>
